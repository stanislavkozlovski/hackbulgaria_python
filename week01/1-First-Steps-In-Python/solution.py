...
def to_digits(number) -> list:
    return list([int(char) for char in str(number)])
...
def count_vowels(string):
    ...
    return sum([value for char, value in chars.items() if char.lower() in vowels])
...
def count_consonants(string):
    ...
    chars = Counter(string)
    return sum([value for char, value in chars.items() if char.lower() in consonants.keys()])
...
def fib_number(number):
    ...
    return int(''.join([str(fib) for fib in fibonacci(number)]))  # inefficient!
print(fib_number(3))
